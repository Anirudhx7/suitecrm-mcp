...
For production, pin to a release tag such as `v3.1.1` instead of floating on `latest`.

```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/anirudhx7/suitecrm-mcp/v3.1.1/docker-compose.yml
```
...
services:
  ...
  suitecrm-mcp-crm1:
    image: ghcr.io/anirudhx7/suitecrm-mcp:v3.1.1
    ports:
      - "3101:3101"
    environment:
